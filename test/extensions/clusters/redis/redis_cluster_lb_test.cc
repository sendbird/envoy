...
TEST_F(RedisLoadBalancerContextImplTest, ClientZone) {
  ...
  RedisLoadBalancerContextImpl context1(
      "foo", true, true, get_request, NetworkFilters::Common::Redis::Client::ReadPolicy::LocalZoneAffinity,
      "us-east-1a");

  EXPECT_EQ("us-east-1a", context1.clientZone());
  EXPECT_EQ(NetworkFilters::Common::Redis::Client::ReadPolicy::LocalZoneAffinity, context1.readPolicy());
...
TEST_F(RedisClusterLoadBalancerTest, LocalZoneAffinityReplicasAndPrimaryWithLocalReplica) {
  ...
  validateAssignment(
      ...
      NetworkFilters::Common::Redis::Client::ReadPolicy::LocalZoneAffinityReplicasAndPrimary, "zone-a");
}
...
TEST_F(RedisClusterLoadBalancerTest, LocalZoneAffinityReplicasAndPrimaryFallbackToReplica) {
  ...
  validateAssignment(
      hosts, expected_assignments, true,
      NetworkFilters::Common::Redis::Client::ReadPolicy::LocalZoneAffinityReplicasAndPrimary, "zone-c");
}
